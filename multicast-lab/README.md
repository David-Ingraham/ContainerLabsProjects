# Multicast Lab with ContainerLab and Automation

Network automation lab demonstrating multicast routing configuration between FRR and GoBGP routers using Ansible and gRPC.
...
### Generated Directory: clab-multicast-lab/
...
```bash
docker exec -it clab-multicast-lab-automation ansible-playbook -i inventory.yml config_playbook.yml
```
...
Expected containers:
- clab-multicast-lab-automation
- clab-multicast-lab-frr1
- clab-multicast-lab-gobgp1
- clab-multicast-lab-host1
- clab-multicast-lab-host2
...
```bash
docker exec clab-multicast-lab-frr1 ps | grep bgp
```
...
```bash
docker exec -it clab-multicast-lab-frr1 vtysh
```

Access GoBGP CLI:

```bash
docker exec clab-multicast-lab-gobgp1 gobgp neighbor
docker exec clab-multicast-lab-gobgp1 gobgp global rib
```
...
```bash
docker exec clab-multicast-lab-automation ssh frruser@10.1.1.11
```
...
- clab-multicast-lab/: Generated directory with topology data and keys
